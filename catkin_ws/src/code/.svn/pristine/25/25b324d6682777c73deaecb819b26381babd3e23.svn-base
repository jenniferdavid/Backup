	...
	sin_betas_({sin(-2.8*M_PI/180), sin(-2*M_PI/180), sin(-1.2*M_PI/180), sin(-0.4*M_PI/180), sin(0.4*M_PI/180), sin(1.2*M_PI/180), sin(2*M_PI/180), sin(2.8*M_PI/180)})
{
	...
	switch (_op_mode)
	{
		case 1: //sdf
			...
			radar_tracks_pub_ = nh_.advertise<cargo_ants_msgs::ObjectTrackAtSdfArray>("radar_tracks", 50);
            gps_pub_ = nh_.advertise<geometry_msgs::Point>("gps_fix", 50);
			markers_tracks_pub_ = nh_.advertise<visualization_msgs::MarkerArray>("radar_markers", 50);
			break; 
		...
		case 2: //ibeo
			...
			for (unsigned int j = 0; j < NUM_IBEO_LAYERS; j++)
			{
				...
				scan_[j].range_min = 0.1;
				scan_[j].range_max = 200;
				scan_[j].angle_min = -60. * M_PI / 180.;//50;
				scan_[j].angle_max = 50. * M_PI / 180.;
				scan_[j].angle_increment = 0.5 * M_PI / 180.;
				N_ranges_ = (unsigned int)( (scan_[j].angle_max - scan_[j].angle_min) / scan_[j].angle_increment );
...
void ROSPublisher::publishSdf(unsigned char *buf, size_t length, const ros::Time& _stamp)
{
	...
	px_ = getSingle(buf, 48, true);//x-pos: Longitudinal position of ego, [m] (egoLongPos in document) 
	py_ = getSingle(buf, 52, true);//y-pos: Lateral position of ego, [m] (egoLatPos in document) 
	th_ = getSingle(buf, 56, true);//heading: heading angle, [rad] (egoHeadingAngle in document) 
	...
	gps_lat = getSingle(buf, 64, true);//gps_lat: GPS latitude [degrees]
    gps_lon = getSingle(buf, 68, true);//gps_lat: GPS latitude [degrees]
...
// 	else if (th_>=M_PI) th_ = -2*M_PI+th_;
    ...
    if (vx < 0.01) //if longitudinal velocity less than 1cm/s, set angular speed to 0
	...
	//Fill tracks and marker array messages
	unsigned char num_objects_uc = buf[16]; //std::cout << "Num Objects uc: " << (unsigned int)num_objects_uc << std::endl; 

	//fill track message header and resize array
	radar_tracks_msg_.header.frame_id = "at_base_link";
	radar_tracks_msg_.header.stamp = ros::Time::now(); 

	//resize message array
    radar_tracks_msg_.tracks.resize(num_objects_uc );
	markers_tracks_msg_.markers.clear();
	markers_tracks_msg_.markers.resize( (unsigned int)num_objects_uc );
	
	//fill arrays of both messages
...
// 		std::cout << "objLatPos: " << ty << std::endl;
		...
		{
			//tracks
			radar_tracks_msg_.tracks[obj_count].id = objId_uint;
			radar_tracks_msg_.tracks[obj_count].id = objVehType_uint;
			radar_tracks_msg_.tracks[obj_count].position.x = tx;
			radar_tracks_msg_.tracks[obj_count].position.y = ty;
			radar_tracks_msg_.tracks[obj_count].position.z = 0.;
			radar_tracks_msg_.tracks[obj_count].velocity.x = tvx;
			radar_tracks_msg_.tracks[obj_count].velocity.y = tvy;
			radar_tracks_msg_.tracks[obj_count].velocity.z = 0.;
			radar_tracks_msg_.tracks[obj_count].heading = theading;
			
			//markers
			markers_tracks_msg_.markers[obj_count].header.frame_id = "at_base_link";
	...
	//resize again with the actual valid num of objects
	radar_tracks_msg_.tracks.resize(obj_count);
	markers_tracks_msg_.markers.resize(obj_count);

	//publish radar objects and markers
	radar_tracks_pub_.publish(radar_tracks_msg_);
	markers_tracks_pub_.publish(markers_tracks_msg_);
    
    //fill GPS fix message
    gps_fix_msg_.x = gps_lat; 
    gps_fix_msg_.y = gps_lon; 
    gps_fix_msg_.z = -1; 
    
    //publish GPS fix message
    gps_pub_.publish(gps_fix_msg_); 
...
bool ROSPublisher::fillLidarData(unsigned char *buf, size_t length)
{
	...
	//return lidar_data_.fillFromDoubles(buf);
}
...
{
    //debugging
    std::vector<unsigned int> n_pts_w(8,0);
	...
	// fill ranges, for each point ... 
    //for (unsigned int i = 0; i<lidar_data_.scanPtLayer.size(); i++)//lidar_data_.scanPts
    for (unsigned int i = 0; i<lidar_data_.scanPts; i++)//lidar_data_.scanPts    
    {
        //check scan point flags: transparent, clutter and, dirt
    	if (lidar_data_.scanPtLayer[i] < 4)// && !(lidar_data_.scanFlags[i] & 0x01) && !(lidar_data_.scanFlags[i] & 0x02) && !(lidar_data_.scanFlags[i] & 0x08))
    	{
            //set layer index at scan_ vector, according up/low device and point layer
            ...
            //get hit index from the azimuth
			idx = (unsigned int)nearbyint( (lidar_data_.scanPtAngle[i] * M_PI / 180. - scan_[layer_idx].angle_min) / scan_[layer_idx].angle_increment );

			// debugging
            if ( (idx >= 0) && (idx < N_ranges_) )
                n_pts_w[layer_idx]++;
                
            //assure idx within the vector limits and store the closer echo
    ...
    // Publish LaserScan messages for the given layer
    for (unsigned int j = begin_layer; j < end_layer; j++)
    {
        scan_pub_[j].publish(scan_[j]);
        //std::cout << "scan " << j << std::endl;
        //std::cout << "\tpoints: " << n_pts_w[j] << " (should be " << N_ranges_  << ")" << std::endl;
    }
}
...
//    sensor_msgs::ChannelFloat32 radialDistance;
    ...
    for (auto ii = 0; ii < lidar_data_.scanPtAngle.size(); ii++)
    {
        assert(lidar_data_.scanPtLayer[ii] < 4 && lidar_data_.scanPtLayer[ii] >=0 && "ROSPublisher::publishPointCloud: Bad scanPtLayer value"); 
        ...
        if (!(lidar_data_.scanFlags[ii] & 0x01) && !(lidar_data_.scanFlags[ii] & 0x02) && !(lidar_data_.scanFlags[ii] & 0x08))
    	{
			if (lidar_data_.upper_layer)
