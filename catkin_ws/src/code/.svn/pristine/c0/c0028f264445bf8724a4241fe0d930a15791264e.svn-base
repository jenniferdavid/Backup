...
Vector qs(4), qe(4), xi, xi_aux, tqs(4), tqe(4);
Vector qstart(2), qend(2);
Vector currentPose(3);
...
Trajectory generateTrajectory(Vector const &xi)
{
 ...
 visualization_msgs::MarkerArray generateMarkers(Trajectory t){
 	...
 	for ( int i = 0; i < t.points.size(); i++) {
 	    ...
 	    if (i == 0)
 	    {       
 	        marker_array_msg.markers[i].color.r = 1;
 	        marker_array_msg.markers[i].color.g = 1;
 	        marker_array_msg.markers[i].color.b = 1;
 	    }
 	    else
 	    {
 	        marker_array_msg.markers[i].color.r = 1;
 	        marker_array_msg.markers[i].color.g = 0;
 	        marker_array_msg.markers[i].color.b = 0.;
 	    }
...
int main(int argc, char **argv)
{
	...
	*/	
    //////////////////////////////////////////////////
	// AUXILIAR CHOMP
    //CHOMP aux_chomp(1.0, 10.0, lambda, 20, 2, numIt, costGain);
	...
	while ( ros::ok() )
	{
		...
		if (tfl_.waitForTransform("odom","agv_base_link", rtime, ros::Duration(5.))  )
		{
			...
			Matrix obs(3,obstacleMap.obstacles.size() + 4);
		    ...
		    //add extra obstacles to avoid bad curves at the begining
		    
			 obs.col(obstacleMap.obstacles.size())     << 7, 10, 10;  
			 obs.col(obstacleMap.obstacles.size() + 1) << 7, -10, 10;			  
			 obs.col(obstacleMap.obstacles.size() + 2) << 3, -10, 10;
			 obs.col(obstacleMap.obstacles.size() + 3) << 0, -10, 10;
		    ...
		    //aux_chomp.initStraightLinePath(qstart, qend);
			...
			//Run CHOMP optimization
			//chomp.generatePath(xi);    	  		    
			double cost1, curv1, cost2, curv2;
			
            chomp.generatePathWithSearchRegion(xi, _Tnew_base, cost1, curv1); //update path 
		    //aux_chomp.generatePathWithSearchRegion(xi_aux, _Tnew_base, cost2, curv2); //update path 
			...
			{
			    xi = xi_aux;
			    cout << "  CHOMP 1 curvature: " << curv2 << endl;
			}
			else
			{
				cout << "  CHOMP 2 curvature: " << curv1 << endl;	
			}*/
			...
			//trajectory_pub.publish(trajectory); //tmp for testing
