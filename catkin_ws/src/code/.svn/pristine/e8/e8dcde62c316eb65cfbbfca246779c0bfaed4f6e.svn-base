...
void AtDriveNode::invKinematics(const Eigen::Vector3d & _pt)
{
    ...
    if ( (xdot != 0.) && (thdot != 0.) )
    {
        ...
        rc = xdot/thdot;
        sc = asin( (axes_separation_)/(rc) );
        dd = axes_separation_/(tan(sc)) - axis_width_/2;
        
        //inner and outer steering
        si = atan( axes_separation_/(dd) );
        so = atan( axes_separation_/((axis_width_+dd)) );
        
        //inner and outer curvature radius
        ri = axes_separation_/(sin(si));
        ro = axes_separation_/(sin(so));
