    ...
    while ( ros::ok() )
    {
		...
		if ( udp_receiver_.blockingReception() != -1 )
		{
			switch( udp_receiver_.packet2message() )
			{
                ...
                case STACKING_PACKETS://nothing to do
                    break;                    
					...
					switch (op_mode_)
					{
						...
						case LIDAR_MODE:
							...
							if (ros_pub_.fillLidarData(udp_receiver_.raw_message_, LIDAR_MESSAGE_LENGTH))
							{
                                ...
                                //ros_pub_.publishPointCloud(stamp_); //WARNING some bug arises in this call
							}
							break;
                ...
                case -1:
                    std::cout << "Error from udp_receiver_.packet2message() call" << std::endl;
                    break;
