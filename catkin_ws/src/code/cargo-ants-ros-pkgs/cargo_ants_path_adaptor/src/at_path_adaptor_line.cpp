 /* AT PATH ADAPTOR WITH 'one' CHOMP INSTANCE AND ADAPTING A STRAIGHT LINE BETWEEEN START AND GOAL
 *
...
int main(int argc, char **argv)
{
    ...
    CHOMP aux_chomp(1.0, 100.0, lambda, 20, 2, numIt, costGain);
	...
	while ( ros::ok() )
	{
		...
		if (tfl_.waitForTransform("odom","at_base_link", rtime, ros::Duration(5.))  )
		{
		    ...
		    */
		    ...
		    //Initialize with a straight path
			...
			//chomp.generatePathWithSearchReg(xi, yaw, cost1, curv1); //update path 
            //aux_chomp.generatePathWithSearchReg(xi_aux, yaw, cost2, curv2); //update path 
