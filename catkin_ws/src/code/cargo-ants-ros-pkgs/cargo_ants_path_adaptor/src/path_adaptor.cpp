...
double CHOMP::chompUpdateWithSearchRegion(Vector  &xi, Eigen::Matrix4d &Tlocal2global, double &U_cost, double &curv)
{  
	...
	Eigen::Matrix4d T_global2local;
	T_global2local = 	Tlocal2global.inverse();			  

	
	//Slope for lines defining search region	
	double m  = 1.73; // e.g. lines at qs_ with 45 deg of slope 
	...
	for (size_t iq (0); iq < nq_; ++iq) 
	{
		...
		for (int ii = 0; ii < OBS_.cols(); ii++) 
		{
			...
			double cost(gain * OBS_(2, ii) * pow(1.0 - dist / OBS_(2, ii), 3.0) / 3.0); 
			...
			if (~( ( xx_l(0) > 0 ) && ( xx_l(1) > m * xx_l(0) ) && (  xx_l(1) < - m * xx_l(0) ) ) )
			{
				...
				cost *= 10;	
				cout << " Outside region, point:  " << xx_l.transpose() << endl;			
			}
			///////////////////////////////////////////////////////	
			*/		
...
double CHOMP::chompUpdateWithCSR(Vector  &xi, double &U_cost, double &curv)
{  
	...
	*/
	...
	{
		Vector const qq (xi_.block (iq * cdim_, 0, cdim_, 1));
	    Vector const qq_init (xinit_.block (iq * cdim_, 0, cdim_, 1)); //reference
		Vector qd;
		if (0 == iq) {
		  qd = 0.5 * (xi_.block ((iq+1) * cdim_, 0, cdim_, 1) - qs_);
		}
		...
		for (int ii = 0; ii < OBS_.cols(); ii++) 
		{
			...
			/*
			Vector Xdiff(qq - qq_init);
		    if( Xdiff.norm() < 6. )
			{
				cout << "***    inside search area with dist: " << Xdiff.norm() << endl;
				//Inside desired search region
				gain = costGain_;
			}
			else
...
void CHOMP::generateGlobalPath(Vector  &xi)
{
	double err;
	bool DYNAMIC_OBS = false;
	
	for (size_t iq (0); iq < nq_; ++iq)  //FOR all points in path
	{
		Vector const qq (xi_.block (iq * cdim_, 0, cdim_, 1));		
		Vector const & xx (qq);
		// Check presence of dynamic obstacles or NEW ones		 
		for (int ii = 0; ii < OBS_.cols(); ii++)  // FOR all obstacles
		{
			Vector delta(xx - OBS_.block(0, ii, cdim_, 1));
			double dist(delta.norm());			
			if (  dist  <  OBS_(2, ii) ) // Check if path is inside NEW obstacle
			{ 
				// path inside NEW obstacle, we need to adapt the path
				DYNAMIC_OBS = true;
			}		
		} 
	}
...
void CHOMP::generatePathWithSearchRegion(Vector  &xi, Eigen::Matrix4d &Tlocal2global, double &U_cost, double &curv)	
{
	double err;
	...
	for (size_t ii(0); ii < numIt_; ++ii)  
	{
		err = CHOMP::chompUpdateWithSearchRegion(xi, Tlocal2global, U_cost, curv);
		if (err < 0.01)
		{
			//it converged
			cter_ = 0;
...
//optimize path using with numIt_ iterations with serch region.	
void CHOMP::generatePathWithCSR(Vector  &xi, double &U_cost, double &curv)	
{
	double err;
	for (size_t ii(0); ii < numIt_; ++ii)  
	{
		err = CHOMP::chompUpdateWithCSR(xi,U_cost,curv);
		if (err < 0.01)
		{
			//it converged
			cter_ = 0;
